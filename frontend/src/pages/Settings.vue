  ...
  <div>
    ...
    <div class="settings-banner">
      ...
      <div style="display:flex;gap:32px">
        <div>
          <div style="font-size:10px;color:rgba(255,255,255,0.5);text-transform:uppercase">历史消耗</div>
          <div style="font-size:18px;font-weight:700;color:#fff">&yen;{{ totalCost }}</div>
        </div>
        <div>
          <div style="font-size:10px;color:rgba(255,255,255,0.5);text-transform:uppercase">请求次数</div>
          <div style="font-size:18px;font-weight:700;color:#fff">{{ totalCalls }}</div>
        </div>
        <div>
          <div style="font-size:10px;color:rgba(255,255,255,0.5);text-transform:uppercase">用户分组</div>
          <div style="font-size:18px;font-weight:700;color:#fff">default</div>
        </div>
      </div>
    </div>
    ...
    <div class="settings-grid">
      ...
      <div class="card">
        ...
        <div class="card-padded" v-if="acctTab === 'bind'">
          ...
          <div class="bind-list">
            <div class="bind-item done">
              <span class="bind-icon">&#x2709;</span>
              <div class="bind-info">
                <div class="bind-name">邮箱</div>
                <div class="bind-detail">{{ userEmail }} <span style="color:var(--success)">已绑定</span></div>
              </div>
              <span class="text-muted" style="font-size:12px">支持修改</span>
            </div>
            <div class="bind-item">
              <span class="bind-icon">&#x1F419;</span>
              <div class="bind-info">
                <div class="bind-name">GitHub</div>
                <div class="bind-detail">待绑定</div>
              </div>
              <button class="btn btn-outline btn-xs">绑定</button>
            </div>
            <div class="bind-item">
              <span class="bind-icon">&#x1F9D1;</span>
              <div class="bind-info">
                <div class="bind-name">LinuxDO</div>
                <div class="bind-detail">待绑定</div>
              </div>
              <button class="btn btn-outline btn-xs">绑定</button>
            </div>
            <div class="bind-item disabled">
              ...
              <div class="bind-info">
                ...
                <div class="bind-detail">未启用</div>
              </div>
            </div>
            <div class="bind-item disabled">
              <span class="bind-icon">&#x1F3AE;</span>
              <div class="bind-info">
                <div class="bind-name">Discord</div>
                <div class="bind-detail">未启用</div>
              </div>
            </div>
          </div>
        ...
        <div class="card-padded" v-else>
          ...
          <div class="form-group">
            ...
            <input v-model="oldPw" type="password" class="form-input" placeholder="输入当前密码" style="max-width:320px" />
          </div>
          <div class="form-group">
            <label class="form-label">新密码</label>
            <input v-model="newPw" type="password" class="form-input" placeholder="至少 8 位，含大小写字母和数字" style="max-width:320px" />
            <p class="form-hint">至少 8 位，需包含大写字母、小写字母和数字</p>
          </div>
          <div class="form-group">
            <label class="form-label">确认密码</label>
            <input v-model="confirmPw" type="password" class="form-input" placeholder="再次输入新密码" style="max-width:320px" />
          </div>
          <div v-if="pwError" class="alert alert-error">{{ pwError }}</div>
          <div v-if="pwSuccess" class="alert alert-success">{{ pwSuccess }}</div>
          <button class="btn btn-primary btn-sm" @click="changePw" :disabled="pwLoading">
            {{ pwLoading ? '保存中...' : '保存' }}
          </button>
        </div>
      </div>
        ...
        <div class="card-padded" v-if="otherTab === 'notify'">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;gap:12px">
              <label class="filter-option"><input type="radio" v-model="notifyMethod" value="email" /> 邮件</label>
              <label class="filter-option"><input type="radio" v-model="notifyMethod" value="webhook" /> Webhook</label>
              <label class="filter-option"><input type="radio" v-model="notifyMethod" value="bark" /> Bark</label>
              <label class="filter-option"><input type="radio" v-model="notifyMethod" value="gotify" /> Gotify</label>
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">额度预警阈值</label>
            <div style="display:flex;align-items:center;gap:8px">
              <span>等价 &yen;</span>
              <input v-model="alertThreshold" class="form-input" style="width:120px" />
              <span class="text-muted">余额低于阈值时触发提醒</span>
            </div>
          </div>
          <div class="form-group">
            <label class="form-label">通知邮箱</label>
            <input class="form-input" placeholder="留空复用绑定邮箱" style="max-width:280px" />
          </div>
          <button class="btn btn-primary btn-sm">保存设置</button>
        </div>
        <div class="card-padded" v-else>
          <div class="empty-state" style="padding:24px">
            <div class="empty-state-text">功能开发中</div>
          </div>
        </div>
      </div>
      ...
      <div class="card card-padded">
        ...
        <div class="form-group">
          ...
          <p class="form-hint">语言设置将同步到所有设备，同时影响 API 报错返回文案</p>
        </div>
        <div class="form-group">
          <label class="form-label">价格显示</label>
          <select class="form-select" style="max-width:200px">
            <option>人民币 (&yen;)</option>
            <option>美元 ($)</option>
          </select>
        </div>
      </div>
...
const acctTab = ref('bind')
const otherTab = ref('notify')
const notifyMethod = ref('email')
const alertThreshold = ref(500000)
const oldPw = ref('')
const newPw = ref('')
const confirmPw = ref('')
const pwError = ref('')
...
onMounted(async () => {
  ...
  try {
    ...
    totalCost.value = d.today_cost_yuan || '0.00'
  } catch (e) { /* */ }
})
</script>
